...
object Pretty:
  private def prettyApp(tm: Tm)(using ns: List[Bind]): String = tm match
    case Tm.App(f, a, Icit.Expl) => s"${prettyApp(f)} ${prettyParen(a)}"
    case Tm.App(f, a, Icit.Impl) => s"${prettyApp(f)} {${prettyParen(a)}}"
    case tm @ Tm.Out(_)          => pretty(tm)
    case f                       => prettyParen(f)

  private def prettyPi(tm: Ty)(using ns: List[Bind]): String = tm match
    case Tm.Pi(Bind.DontBind, Icit.Expl, t, b) =>
      s"${prettyParen(t, true)} -> ${prettyPi(b)(using Bind.DontBind :: ns)}"
    case Tm.Pi(x, i, t, b) =>
      s"${i.wrap(s"$x : ${pretty(t)}")} -> ${prettyPi(b)(using x :: ns)}"
  ...
  private def prettyLam(tm: Tm)(using ns: List[Bind]): String =
    def go(tm: Tm, first: Boolean = false)(using ns: List[Bind]): String =
      tm match
        case Tm.Lam(x, Icit.Expl, _, b) =>
          s"${if first then "" else " "}$x${go(b)(using x :: ns)}"
        case Tm.Lam(x, Icit.Impl, _, b) =>
          s"${if first then "" else " "}{$x}${go(b)(using x :: ns)}"
  ...
  def prettyParen(tm: Tm, app: Boolean = false)(using
    ...
    tm match
      ...
      case Tm.Meta(_)             => pretty(tm)
      case Tm.AppPruning(_, _)    => pretty(tm)
      case Tm.App(_, _, _) if app => pretty(tm)
  ...
  def pretty(tm: Tm)(using ns: List[Bind]): String =
    tm match
      ...
      case Tm.Meta(m)          => s"?$m"
      case Tm.AppPruning(m, _) => s"?$m"
      case Tm.Let(x, t, v, b)  =>
        s"let $x : ${pretty(t)} = ${pretty(v)}; ${prettyLift(x.toBind, b)}"

      case Tm.Type => "Type"

      case Tm.Pi(_, _, _, _)  => prettyPi(tm)
      case Tm.Lam(_, _, _, _) => prettyLam(tm)
      case Tm.App(_, _, _)    => prettyApp(tm)
