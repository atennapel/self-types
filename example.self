def Bool = self b => (P : Bool -> Type) -> P True -> P False -> P b
def True = in \P t f => t
...
def Nat = self n => (P : Nat -> Type) -> P Z -> ((m : Nat) -> P (S m)) -> P n
def Z = in \P z s => z
...
def Vec n A : Type =
  ...
  (P : {n} -> Vec n A -> Type) ->
  P VNil ->
  ({m} (hd : A) (tl : Vec m A) -> P (VCons hd tl)) ->
  P {n} v
...
def Id {A} (x y : A) : Type = self id => (P : {y} -> Id x y -> Type) -> P Refl -> P {y} id
